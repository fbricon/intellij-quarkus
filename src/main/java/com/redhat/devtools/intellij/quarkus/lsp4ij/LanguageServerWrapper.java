...
public class LanguageServerWrapper {
    ...
    public synchronized void start() throws IOException {
        ...
        if (this.initializeFuture == null) {
            final boolean shouldLog = LoggingStreamConnectionProviderProxy.shouldLog(serverDefinition.id);
            ...
            this.initializeFuture = CompletableFuture.supplyAsync(() -> {
                ...
                try {
                    // Starting process...
                    if (shouldLog) {
                        LanguageServerLifecycleManager.getInstance()
                                .onStartingProcess(this);
                    }
                    lspStreamProvider.start();
                    // End process with success
                    if (shouldLog) {
                        LanguageServerLifecycleManager.getInstance()
                                .onStartedProcess(this, null);
                    }
                } catch (IOException e) {
                    // End process with error
                    if (shouldLog) {
                        LanguageServerLifecycleManager.getInstance()
                                .onStartedProcess(this, e);
                    }
                ...
                this.launcherFuture = launcher.startListening();
                    ...
                    .thenAccept(res -> {
                        ...
                        if (shouldLog) {
                            LanguageServerLifecycleManager.getInstance()
                                    .onStartedLanguageServer(this, e);
                        }
                        stop();
                        return null;
                    });
    ...
    private void logMessage(Message message) {
        LanguageServerLifecycleManager.getInstance().logLSPMessage(message, this);
    ...
    synchronized void stop() {
        ...
        Runnable shutdownKillAndStopFutureAndProvider = () -> {
            ...
            LanguageServerLifecycleManager.getInstance()
                    .onStoppedLanguageServer(this, null);
